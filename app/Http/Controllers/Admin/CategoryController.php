...
class CategoryController extends Controller
{
    ...
    public function store(Request $request): \Illuminate\Http\JsonResponse
    {
        ...
        $response = $this->categoryRepository->store($attributes);
    ...
    {
        $resultDepartment = $this->categoryRepository->find(base64_decode($id));
    ...
    {
        $attributes = $request->all();
        $response = $this->categoryRepository->update(base64_decode($id),$attributes);
        if($response['status'] == TRUE) {
            return response()->json(array('status' => true, 'message' => $response['message']));
        } else {
            return response()->json(array('status' => false, 'message' => $response['message']));
    ...
    {
        $response = $this->categoryRepository->destroy(base64_decode($id));
        if($response['status'] == TRUE) {
            return response()->json(array('status' => true, 'message' => $response['message']));
        } else {
            return response()->json(array('status' => false, 'message' => $response['message']));
    ...
    public function getCategories (Request $request): \Illuminate\Http\JsonResponse
    {
        ...
        switch ($orderColumn) {
            ...
            case '1': $query->orderBy('number_of_question', $orderDirection); break;
